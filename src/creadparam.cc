...
static float *MirrorSpot;                    //@< the light spot of the mirrors 
...
static float *Nsb;                         //@< the night sky background
static float *TriggerSigma;                 //@< the ratio of signal to noise;
static float *InnerFixTriggerThreshold;     //@< if NSB is not simulated, the fix trigger threishold is used 
static float *OuterFixTriggerThreshold;      //@ thriggerthreshold of edge sipms
static int WLFormat=0;  // Wavelength format output by  corsika
static int AtmModel=2; // Atm model; 2 is for US standard, 3 is for Magic summer, 4 is for Magic winter
static float Altitude=440000; //the altitude of the obervatory (cm)
...
void WReadConfig::readparam(char * filename)
{
  ...
  MirrorSpot = new float [MAX_NUMBER_OF_CTS];
  Nsb = new float [MAX_NUMBER_OF_CTS];
  TriggerSigma  = new float [MAX_NUMBER_OF_CTS];
  InnerFixTriggerThreshold = new float [MAX_NUMBER_OF_CTS];
  OuterFixTriggerThreshold = new float [MAX_NUMBER_OF_CTS];
  ...
  for (i = 0; i < MAX_NUMBER_OF_CTS ; i++){
     ...
     Nsb[i] = 0;
     TriggerSigma[i] = 0;
     InnerFixTriggerThreshold[i] = 0;
     OuterFixTriggerThreshold[i] = 0; 
  }  
 ...
 while (! is_end) {
   ...
   switch ( i ) {
     ...
     case wavelength:
         sscanf(line, "%s  %s %d", token, token2,&k);   
     ...
     case fadc_bin_length:
         ...
         break;
     case nsb:
         sscanf(line,"%s %s %d %f %f %f",token,token2,&k,&x, &y,&z);
         if(strcmp(token2,"T")==0||strcmp(token2,"TRUE")==0){
           NsbFlag = TRUE;
           Nsb[k] = x; 
           TriggerSigma[k] = y;
           ...
           NsbFlag = FALSE;
           Nsb[k] = x;
           InnerFixTriggerThreshold[k] = y;
           OuterFixTriggerThreshold[k] = z;
         }
         break;
     case atmmodel:
       sscanf(line,"%s %d %f",token,&k,&x);
...
float WReadConfig::GetMirrorSpot(int ict)
{ 
   return MirrorSpot[ict];
}
...
float WReadConfig::GetNSB(int ict)
{
  return Nsb[ict];
}
...
float WReadConfig::GetInnerFixTriggerThreshold(int ict)
{
   return InnerFixTriggerThreshold[ict];
}
float WReadConfig::GetOuterFixTriggerThreshold(int ict)
{
   return OuterFixTriggerThreshold[ict];
}
float WReadConfig::GetTriggerSigma(int ict)
{
   return TriggerSigma[ict];
}
...
int WReadConfig::GetWLFormat()
{
  return WLFormat;
}

int WReadConfig::GetAtmModel()
{
  return AtmModel;
}
float WReadConfig::GetAltitude()
{
  return Altitude;
}
